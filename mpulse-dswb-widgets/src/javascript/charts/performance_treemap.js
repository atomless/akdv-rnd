;((window, $, d3, moment, dataProviderFactory,
   ...
   math_utils, css_utils, string_utils, dom_utils, env_utils, data_utils, event_utils, lang_utils) => {
    ...
    const EVENT_NAMESPACE_CHART_DATA_UPDATE = 'akdv-treemap';
    ...
    const CATEGORICAL_DIMENSION_PROPERTY_NAME = 'devicetypename';
    const LINEAR_DIMENSION_PROPERTY_NAME = 'timer';
    const ANNOTATION_CONTAINER_ID = '#annotation-layer';
    const CONTAINER_ID = '#akdv-treemap';

    const data_hierarchy_array = env_utils.getQueryParamValue('hierarchy')
        ? env_utils.getQueryParamValue('hierarchy').split(',') 
        : [CATEGORICAL_DIMENSION_PROPERTY_NAME, 'pagegroupname', 'useragentname', 'countrycode'];
    ...
    let linear_scale_color_max = Number(env_utils.getQueryParamValue('color_max') || 20000 );
    let linear_scale_color_min = Number(env_utils.getQueryParamValue('color_min') || 1000);
    ...
    const AXIS_LABEL_TEXT = categorical_colors_map? 'Media Type' : 'Load Time';
    ...
    const onInitCharts = () => {
        ...
        const percentileInput = customNumberInputFactory({ 
            ...
            default_value: data_utils.getValueAsNumberOrFallbackValue(mpulse.getFilterValueByName('percentile'), 50), 
        ...
        const annotationBubble = componentFactory({ 
            ...
            function_to_format_data: ({ d, end_node_property_name } = {}) => {
                if (!d.children) {
                    let timer_val = string_utils.getFormattedInteger(d.data[LINEAR_DIMENSION_PROPERTY_NAME]);
                    ...
                    return { 
                        title_obj: { 
                            name: lang_utils.getLocalizedHTMLString(end_node_property_name, 'AKDV.Columns'),
                            value: d.data[end_node_property_name]
                        }, 
                        name_value_unit_obj_list: [
                            { name: 'beacons', label: 'Total Beacons', value: string_utils.getFormattedInteger(d.data.beacons) } ,
                            { name: 'pageloadtime', label: (mpulse.getFilterDisplayLabelByName(LINEAR_DIMENSION_PROPERTY_NAME) || (dataProvider.meta.timer === 'pageloadtime' ? 'Load Time' : dataProvider.meta.timer) || 'Load Time') + show_percentile, value: timer_val, unit: (timer_val === 'unknown ')? '' : 'ms' }
                        ],
                    ...
                    return { 
                        ...
                        name_value_unit_obj_list: [
                            { name: 'beacons', label: 'Total Beacons', value: string_utils.getFormattedInteger(d.value) } ,
                        ],
                        x: container_el.getBoundingClientRect().left + d.x0 + ((d.x1 - d.x0) * 0.5),
        ...
        const updateData = () => {
            let timer_text = (mpulse.getFilterDisplayLabelByName(LINEAR_DIMENSION_PROPERTY_NAME) || (dataProvider.meta.timer === 'pageloadtime' ? 'Load Time' : dataProvider.meta.timer) || 'Load Time') + ' (ms)';
            let w = container_el.offsetWidth, h = container_el.offsetHeight;
            let label_text = (!categorical_colors_map && dataProvider.meta && dataProvider.meta.timer)
                ? timer_text 
                : undefined;
            ...
            tm.contextmenu.sections = [{ 
                ...
                items: [
                    { 
                        ...
                        onclick_fn : e => event_utils.dispatchCustomEvent(window, 'mpulse-open-linked-dashboard', 'akdv', [dashboard_path])
                    }
                ]
            }];
        };
        ...
        $(window).on(`data-updated.${EVENT_NAMESPACE_CHART_DATA_UPDATE}`, e => updateData());
