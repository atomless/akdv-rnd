...
const perf_map = new Map(Object.entries({
    ...
    'akdv.chartRenderStart_chartRenderComplete': 1000,
...
const test_data_file = 'performance_treemap.json';

test.serial('PAGE LOADED', t => common_tests.page_load_test(t, 'AKDV PERF : TREEMAP CHART PAGE', `${config.base_uri}html/${test_chart_html}?debug=${config.browser_logging_level}&data=${test_data_file}`));
test.serial('IS PRODUCTION ENVIRONMENT', common_tests.is_production_test);

test.serial('PERFORMANCE REGRESSION TEST - PERF-TREEMAP CHART:', t => common_tests.performance_regression(t, perf_map));
